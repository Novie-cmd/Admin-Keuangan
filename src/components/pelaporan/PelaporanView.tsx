...
export const PelaporanView: React.FC<PelaporanViewProps> = ({
  ...
  // Helper calculation for Laporan Per Program
  const programReportData = programs
    .filter(p => p.tahun === selectedTahun)
    .map(p => {
      const pagu = currentAnggaran
  ...
  const kegiatanReportData = kegiatanList
    ...
    .map(k => {
      ...
      return {
        kodeProg: k.kodeProgram,
        kodeKeg: k.kodeKegiatan,
        namaKeg: k.namaKegiatan,
        pagu,
        realisasi: real,
        sisa,
        persen: pct
  ...
  const subReportData = subKegiatanList
    .filter(s => s.tahun === selectedTahun)
    .map(s => {
      const pagu = currentAnggaran
        .filter(a => a.kodeSub === s.kodeSub)
        .reduce((s, a) => s + a.paguAkhir, 0);

      const real = currentRealisasi
        .filter(r => r.kodeSub === s.kodeSub)
        .reduce((s, r) => s + r.nilai, 0);
  ...
  const belanjaReportData = belanjaList.map(b => {
    const pagu = currentAnggaran
      .filter(a => a.kodeBelanja === b.kodeBelanja)
      .reduce((s, a) => s + a.paguAkhir, 0);

    const real = currentRealisasi
      .filter(r => r.kodeBelanja === b.kodeBelanja)
      .reduce((s, r) => s + r.nilai, 0);
  ...
  const exportToExcel = () => {
    ...
    if (activeTab === 'laporan-program') {
      ...
      exportRows = programReportData.map(r => ({
        'Kode Program': r.kode,
        'Nama Program': r.nama,
        'Anggaran Pagu (Rp)': r.pagu,
        'Realisasi (Rp)': r.realisasi,
        'Sisa Anggaran (Rp)': r.sisa,
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-950 print:bg-slate-200 text-slate-300 print:text-slate-900 font-bold uppercase border-b border-slate-800">
                  <tr>
                    ...
                    <th className="p-3 text-right">Anggaran Pagu (Rp)</th>
                ...
                <tbody className="divide-y divide-slate-800 print:divide-slate-300">
                  {programReportData.map(r => (
                    <tr key={r.kode} className="hover:bg-slate-800/40">
                      ...
                      <td className="p-3 text-right font-mono">Rp {r.pagu.toLocaleString('id-ID')}</td>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950 print:bg-slate-200 text-slate-300 print:text-slate-900 font-bold uppercase border-b border-slate-800">
                  <tr>
                    ...
                    <th className="p-3 text-right">Pagu (Rp)</th>
                    <th className="p-3 text-right">Realisasi (Rp)</th>
